// Package pgstore implements a Postgres-backed UndoStore.
//
...
// Open connects to postgres at dsn, creates tables if needed, and
// returns a PgStore keyed by filePath.
//
// dsn format: "host=localhost user=postgres dbname=editor sslmode=disable".
func Open(dsn, filePath string, initial piece.Snapshot) (*PgStore, error) {
...
func (s *PgStore) Push(snap piece.Snapshot) error {
	// Discard redo branch.
...
func (s *PgStore) Undo() (piece.Snapshot, bool) {
	if s.stackTop <= 0 {
...
func (s *PgStore) Redo() (piece.Snapshot, bool) {
	if s.stackTop+1 >= len(s.entries) {
		return piece.Snapshot{}, false
	}
	s.stackTop++
...
func (s *PgStore) Current() piece.Snapshot {
	if s.stackTop < 0 || s.stackTop >= len(s.entries) {
		return piece.Snapshot{}
	}
...
type snapshotJSON struct {
	Pieces    []piece.Piece `json:"pieces"`
	AddLen    int           `json:"add_len"`
	CursorRow int           `json:"cursor_row,omitempty"`
	CursorCol int           `json:"cursor_col,omitempty"`
}

func marshalSnapshot(s piece.Snapshot) ([]byte, error) {
	return json.Marshal(snapshotJSON(s))
}

func unmarshalSnapshot(data []byte) (piece.Snapshot, error) {
	var v snapshotJSON
	if err := json.Unmarshal(data, &v); err != nil {
		return piece.Snapshot{}, err
	}
	return piece.Snapshot(v), nil
}
